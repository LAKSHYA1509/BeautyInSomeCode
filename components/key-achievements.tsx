  ...
  return (
    <section id="achievements" className="py-24 md:py-32 relative bg-background overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 relative z-10">
        ...
        >
          <span className="text-accent text-sm font-mono uppercase tracking-widest block mb-4">Impact & Results</span>
          <h2 className="text-4xl md:text-5xl lg:text-6xl font-light text-foreground">
            Key <span className="text-accent font-semibold">Achievements</span>
          </h2>
        </motion.div>
...
function AchievementCard({ item, index }: { item: any, index: number }) {
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      className="group relative p-1 rounded-2xl bg-gradient-to-br from-white/10 to-transparent hover:from-accent/20 hover:to-primary/20 transition-all duration-500"
    >
      <div className="relative h-full bg-card/90 backdrop-blur-xl p-8 rounded-xl border border-white/5 group-hover:border-accent/20 transition-all duration-500 overflow-hidden">

        {/* Large Icon Background */}
        <item.icon className="absolute -bottom-8 -right-8 w-40 h-40 text-foreground/5 group-hover:text-accent/5 transition-colors duration-500 transform rotate-12" />
        ...
        <div className="relative z-10">
          ...
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-accent/10 border border-accent/20 text-accent font-semibold text-sm">
            {item.impact}
          </div>
        </div>
      </div>
