...
export function AchievementsSection() {
  ...
  return (
    <section id="achievements" className="relative py-32 md:py-40 overflow-hidden">
      {/* Background accent */}
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] rounded-full bg-[#C9A962]/3 blur-[200px]" />
      </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 mb-20">
          {achievements.map((achievement, index) => (
            <motion.div
              ...
              className="group relative p-8 rounded-2xl bg-[#1A1A1A]/50 border border-[#1A1A1A] hover:border-[#2A2A2A] backdrop-blur-sm transition-all duration-500"
            >
              ...
              <div className="relative z-10 text-center">
                <div className="text-4xl md:text-5xl font-light text-[#E8E8E8] mb-2">
                  <AnimatedCounter value={achievement.number} suffix={achievement.suffix} />
                </div>
                <div className="text-[#E8E8E8] font-medium mb-1">{achievement.label}</div>
                <div className="text-sm text-[#888888]">{achievement.description}</div>
              </div>
            </motion.div>
          ))}
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {milestones.map((milestone, index) => (
            <motion.div
              ...
              className="group relative p-8 rounded-2xl border border-[#1A1A1A] hover:border-[#8B7EC8]/30 transition-all duration-500"
            >
              ...
              <div className="relative z-10">
                <div className="w-12 h-12 rounded-full bg-gradient-to-br from-[#8B7EC8]/20 to-[#4A6FA5]/20 flex items-center justify-center mb-6">
                  <div className="w-3 h-3 rounded-full bg-[#C9A962]" />
                </div>
                <h3 className="text-xl font-medium text-[#E8E8E8] mb-2 group-hover:text-[#C9A962] transition-colors duration-300">
                  {milestone.title}
                </h3>
